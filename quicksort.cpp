...
std::vector<int> quickSort(std::vector<int> sequence, char const *mode = "m")
{
    ...
    else
    {
        ...
        if (std::tolower(mode[0]) == 'm')
        {
            int middleIndex = (int)std::round(length / 2);
            int a = sequence[0];
            int b = sequence[middleIndex];
            int c = sequence[length - 1];
            ...
            else if (b == sequence[middleIndex])
            {
                sequence.erase(sequence.begin() + middleIndex);
            }
            else if (b == sequence[length - 1])
            {
                sequence.erase(sequence.begin() + length - 1);
            }
        }
        else
        {
        ...
        less = quickSort(less, mode);
        more = quickSort(more, mode);
